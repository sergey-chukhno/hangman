...
def load_hangman_images(location):
    ...
    for i in range(7):
        # Construct absolute path using current directory
...
guessed = []
current_topic = ""
images = []  # Will be loaded after location selection
...
def draw_menu(selected_option, options, menu_type="main"):
    ...
    title_text = "HANGMAN" if menu_type == "main" else "Select game level" if menu_type == "level" else "Select topic" if menu_type == "topic" else "Select location"
