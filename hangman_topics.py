...
def draw():
    ...
    window.blit(timer_display, (780 - timer_display.get_width(), HEIGHT - 50))

    topic_text = f"Topic: {current_topic}"
    topic_display = LETTER_FONT.render(topic_text, 1, BLACK)
    window.blit(topic_display, (20, HEIGHT - 50))
...
def draw_menu(selected_option, options, menu_type="main"):
    ...
    title_text = "HANGMAN" if menu_type == "main" else "Select game level" if menu_type == "level" else "Select topic"
...
def main():
    global wrong_guesses, guessed, word, start_time, letters, TIME_LIMIT, current_topic
